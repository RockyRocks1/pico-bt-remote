#define BTSTACK_FILE__ "BlePico.cpp"
#include "BlePico.hpp"
...
// Setup structural tracking registration parameters for the low-level loops
static btstack_packet_callback_registration_t hci_event_callback_registration;
static btstack_packet_callback_registration_t sm_event_callback_registration;
BlePico* BlePico::instance = nullptr;
BlePico* BlePico::GetInstance() {
    if (!BlePico::instance)
        BlePico::instance = new BlePico();
    return BlePico::instance;
}

void BlePico::Initialize() {
    btstack_memory_init();
    ...
    hci_event_callback_registration.callback = &BlePico::PacketHandler;
    hci_add_event_handler(&hci_event_callback_registration);

    sm_event_callback_registration.callback = &BlePico::PacketHandler;
    sm_add_event_handler(&sm_event_callback_registration);

    hids_device_register_packet_handler(&BlePico::PacketHandler);
    hci_power_control(HCI_POWER_ON);

    
}
void BlePico::PollUsbStream() {
    if (m_hidCid == HCI_CON_HANDLE_INVALID)
        return;

    while (true) {
        static UsbPacket incomingPacket;
        static size_t bytesRead = 0;
        uint8_t* packetBuffer = (uint8_t*)&incomingPacket;

        int byteIn = getchar_timeout_us(0);
        if (byteIn == ATT_ERROR_TIMEOUT)
            break;

        packetBuffer[bytesRead++] = (uint8_t)byteIn;

        if (bytesRead == sizeof(UsbPacket)) {
            bytesRead = 0;
            ...
            case CMD_MOUSE_REPORT:
                SendMouseReport(incomingPacket.data.mouse);
                break;
            case CMD_KEYBOARD_REPORT:
                SendKeyboardReport(incomingPacket.data.keyboard);
                break;
            default:
                break;
            }
...
void BlePico::SendMouseReport(MousePayload& mouse) {
    if (!CanSendReport())
        return;
...
void BlePico::SendKeyboardReport(KeyboardPayload& keyboard) {
    if (!CanSendReport())
        return;
...
void BlePico::PacketHandler(uint8_t packetType, uint16_t channel, uint8_t* packet, uint16_t size) {
    GetInstance()->HandleBluetoothEvent(packetType, packet);
}
static absolute_time_t next_action_time = make_timeout_time_ms(100);
static bool move_right = true;
static bool move = false;
void BlePico::HandleBluetoothEvent(uint8_t packetType, uint8_t* packet) {
